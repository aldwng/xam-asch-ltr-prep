...
export HADOOP_OPTS="-Dhadoop.property.hadoop.client.keytab.file=/etc/h_bs.keytab \
                    -Dhadoop.property.hadoop.client.kerberos.principal=h_ns@XMH"
...
then
  ...
  ${HADOOP} ${ZJY_CLUSTER} fs -put -f ${model} /user/h_ms/xam/lambdamart/model/model.txt
  ${HADOOP} ${ZJY_CLUSTER} fs -put -f ${model} /user/h_ms/xam/lambdamart/model/model.txt.${day}
